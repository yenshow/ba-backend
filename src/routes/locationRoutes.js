...
router.get("/floors", noCache, asyncHandler(async (req, res) => {
  ...
  const result = await locationService.getFloors(filters);
...
router.get("/floors/:id", noCache, validateIntegers("id"), asyncHandler(async (req, res) => {
  const { id } = req.params;
  const { locationType } = req.query;
  const result = await locationService.getFloorById(parseInt(id), locationType || null);
  res.sendSuccess(result);
}));

// 建立樓層（需要認證）
router.post("/floors", authenticate, asyncHandler(async (req, res) => {
  const result = await locationService.createFloor(req.body, req.user.id);
...
router.put("/floors/:id", authenticate, validateIntegers("id"), asyncHandler(async (req, res) => {
  const { id } = req.params;
  const result = await locationService.updateFloor(parseInt(id), req.body, req.user.id);
  res.sendSuccess(result);
}));
...
router.delete("/floors/:id", authenticate, validateIntegers("id"), asyncHandler(async (req, res) => {
  const { id } = req.params;
  const result = await locationService.deleteFloor(parseInt(id));
  res.sendSuccess(result);
}));
