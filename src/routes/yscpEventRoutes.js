 ...
 *
 * 此端點用於接收 YSCP 系統推送的事件（人流統計、車輛進出等）。
 * 僅回傳 200 與 code "0" 作為收訖確認，不向 YSCP 回傳業務資料。
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      await yscpEventService.handleEvent(eventData);
      res.status(200).json({
        code: "0",
        msg: "Success",
        data: {},
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      const personIds = await yscpPersonService.getPersonIdsFromFile(filePath);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      const result = await yscpPersonService.getPersonPicture(personId, picUri);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      const results = await yscpPersonService.getPersonInfoFromFile(filePath, {
        includePicture,
      });
