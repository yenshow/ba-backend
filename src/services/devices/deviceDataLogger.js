...
class DeviceDataLogger {
  ...
  /**
   * 從 sensorParameters 自動產生 logging values（感測器預設自動記錄）
   */
  _buildLoggingValuesFromSensorParameters(sensorParameters) {
    ...
    try {
      ...
      if (typeCode === "sensor") {
        ...
        if (!values || values.length === 0) {
          ...
          values = this._buildLoggingValuesFromSensorParameters(sensorParams);
        }
      ...
      setTimeout(() => this.configCache.delete(deviceId), 5 * 60 * 1000);
