...
const { getPermissionCodesForDeployment } = require("./catalog");
...
async function attachEffectivePermissions(req) {
  ...
  const { codes } = await getEffectivePermissionsForUser(
    req.user.id,
    req.user.role,
  );
  req.user.permissions = codes;
...
async function replaceUserPermissionOverrides(userId, overrides, clientQuery) {
  await clientQuery("DELETE FROM user_permission_overrides WHERE user_id = ?", [
    userId,
  ]);
  for (const o of overrides) {
